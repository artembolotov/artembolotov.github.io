...
В корне репозитория лежит ещё один файл — `.rsyncignore`. Это простой список того, что не нужно отправлять на сервер, даже если оно случайно оказалось в папке `www`:
