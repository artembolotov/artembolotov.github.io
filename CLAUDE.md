...
- **Liquid runs over the whole `.md` file before kramdown, including inside fenced/inline code.** Any literal `{{ ... }}` or `{% ... %}` in example code (GitHub Actions `${{ vars.X }}` / `${{ secrets.X }}`, Jinja, other templating) gets parsed as a Liquid tag and silently swallowed/emptied — it does NOT render as-is just because it's in a code block. Wrap such snippets in `{% raw %}...{% endraw %}` (whole fenced block, or inline around a single `` `${{ ... }}` `` span). See [blog/_posts/2026-08-24-git-push-deploy.md](blog/_posts/2026-08-24-git-push-deploy.md) for both cases. When adding or editing a post with example code, grep it for `{{` / `${{` before publishing.
